...
https://awesome-copilot.github.com/instructions/#file=instructions%2Frust.instructions.md

I may make edits to it over time, but the original can be found above.
- Connor
-->
...
* [The broader Rust community](https://users.rust-lang.org).
...
* Please ensure code compiles with *absolutely zero* warnings.

## Dependencies

In general, minimize the number of crate dependencies unless:

* The implementation of custom logic to avoid using a crate distracts from the goal of the overall project.
* You are requested to use a specific crate.
* You strongly believe the usage of the crate will improve the quality of the project.
...
* Don't use `unwrap()` or `expect()` unless absolutely necessary—prefer proper error handling.
    * Instead, propagate errors up through the call stack using `Result<T, E>` or similar types.
