...
function __shuggtool_dvchk_usage()
{
    echo "Dev check: searches your code for TODOs and other keywords."
    echo ""
    echo "Usage: dvchk [-f FILE] [-d DIRECTORY]"
...
function __shuggtool_dvchk_search_file()
{
    f=$1
...
function __shuggtool_dvchk_search_dir()
{
    d=$1
    ...
    for path in $(find ${d} -type f); do
        __shuggtool_dvchk_search_file "${path}"
    done
}

# Main function.
function __shuggtool_dvchk()
{
    ...
    if [ ! -z "${file}" ]; then
        __shuggtool_dvchk_search_file "${file}"
        return 0
    fi

    # if a directory was specified, we'll recursively search it
    if [ ! -z "${dir}" ]; then
        __shuggtool_dvchk_search_dir "${dir}"
        return 0
    fi
